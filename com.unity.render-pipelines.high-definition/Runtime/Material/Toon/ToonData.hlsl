...
{
    float2 toonColorMapUv = TRANSFORM_TEX(input.texCoord0.xy, _ToonColorMap);
    surfaceData.color = SAMPLE_TEXTURE2D(_ToonColorMap, sampler_ToonColorMap, toonColorMapUv).rgb * _ToonColor.rgb;
    float alpha = SAMPLE_TEXTURE2D(_ToonColorMap, sampler_ToonColorMap, toonColorMapUv).a * _ToonColor.a;
    ...
    // Light Layers are currently not used for the Toon shader (because it is not lit)
    // But Toon objects do cast shadows according to their rendering layer mask, which is what we want to
    ...
    {
        surfaceData.color = GetTextureDataDebug(_DebugMipMapMode, toonColorMapUv, _ToonColorMap, _ToonColorMap_TexelSize, _ToonColorMap_MipInfo, surfaceData.color);
    }
#endif
