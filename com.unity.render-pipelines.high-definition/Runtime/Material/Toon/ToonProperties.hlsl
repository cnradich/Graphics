...
TEXTURE2D(_ToonColorMap);
SAMPLER(sampler_ToonColorMap);
...
float4  _ToonColor;
float4 _ToonColorMap_ST;
float4 _ToonColorMap_TexelSize;
float4 _ToonColorMap_MipInfo;
...
UNITY_DOTS_INSTANCING_START(MaterialPropertyMetadata)
    UNITY_DOTS_INSTANCED_PROP(float4, _ToonColor);
    UNITY_DOTS_INSTANCED_PROP(float3, _EmissiveColor);
    UNITY_DOTS_INSTANCED_PROP(float , _AlphaCutoff);
UNITY_DOTS_INSTANCING_END(MaterialPropertyMetadata)

#define _ToonColor     UNITY_ACCESS_DOTS_INSTANCED_PROP_FROM_MACRO(float4, Metadata__ToonColor)
