...
#ifndef UNIVERSAL_TERRAIN_TOON_PASSES_INCLUDED
#define UNIVERSAL_TERRAIN_TOON_PASSES_INCLUDED
...
#include "Packages/com.unity.render-pipelines.universal/Shaders/Stylized/Toon/Toon.hlsl"
...
void InitializeInputData(Varyings input, half3 normalTS, out ToonInputData inputData)
{
    inputData.keyLight = GetMainLight(TransformWorldToShadowCoord(input.positionWS.xyz));
    inputData.aoFactor = GetScreenSpaceAmbientOcclusion(GetNormalizedScreenSpaceUV(input.clipPos));
...
#endif
{
    ...
    InputData gbuffInputData;
    gbuffInputData.positionWS = inputData.positionWS;
    gbuffInputData.fogCoord = inputData.fogCoord;
    gbuffInputData.shadowCoord = inputData.shadowCoord;
    gbuffInputData.shadowMask = inputData.shadowMask;
    gbuffInputData.bakedGI = inputData.bakedGI;
    gbuffInputData.normalWS = inputData.normalWS;
    gbuffInputData.viewDirectionWS = inputData.viewDirectionWS;

    return BRDFDataToGbuffer(brdfData, gbuffInputData, smoothness, color.rgb);
    ...
    surfaceData.shadowColor = surfaceData.baseColor * half3(0.5, 0.5, 0.5);
    surfaceData.specular = half3(0.0h, 0.0h, 0.0h);
    surfaceData.emission = half3(0.0h, 0.0h, 0.0h);
    surfaceData.occlusion = occlusion;
    surfaceData.alpha = alpha;
    surfaceData.numLightLevels = 1;
    surfaceData.terminatorBias = 0;

    half4 color = Toon(inputData, surfaceData); // UniversalFragmentPBR(inputData, albedo, metallic, /* specular */ half3(0.0h, 0.0h, 0.0h), smoothness, occlusion, /* emission */ half3(0, 0, 0), alpha);
